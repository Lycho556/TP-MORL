#!/usr/bin/env bash
# ============================================================================
#  TP-MORL 批次 v15 —— 一条命令跑完全部实验
#
#  在服务器上只需要敲这一句：
#
#      bash ~/TP-MORL/启动实验_v15.sh
#
#  想挂后台断开 ssh 也不中断：
#
#      nohup bash ~/TP-MORL/启动实验_v15.sh > ~/v15.log 2>&1 &
#      tail -f ~/v15.log
#
#  脚本自己会做：git pull → 环境自检 → 逐组训练 → 每组跑完顺手算规则基线
#  → commit+push+打标签 → 断点续跑（已完成的组自动跳过）→ 全部完成后重算
#  实施类指标、出主判据、打标签 batch-v15-complete。
#
#  ---------------------------------------------------------------------------
#  v15 为什么存在
#
...
# ---------------------------------------------------------------------------
SCENARIOS=(
  "1|main|主组：生命周期观测全开|--budget 900 --carry 3 --horizon-eval auto"
  "2|ablation|消融：生命周期 6 维特征置零（与第 1 组仅差此项，共用分母）|--budget 900 --carry 3 --horizon-eval auto --no-obs-lifecycle"
  "3|staged|资金口径 staged：立项付 20pct，Cost 按支付年计入|--budget 900 --carry 3 --horizon-eval auto --budget-mode staged --stage-init 0.2"
  "4|carry1|结转上限 1 年（最紧）|--budget 900 --carry 1 --horizon-eval auto"
  "5|carry6|结转上限 6 年|--budget 900 --carry 6 --horizon-eval auto"
  "6|carryInf|结转无上限（钱可以一直攒）|--budget 900 --carry 999 --horizon-eval auto"
  "7|budget600|年度总额 600（更紧）|--budget 600 --carry 3 --horizon-eval auto"
  "8|budget1500|年度总额 1500（更松）|--budget 1500 --carry 3 --horizon-eval auto"
  "9|tauA1|平均审批 1 年（最快）|--budget 900 --carry 3 --horizon-eval auto --tau-approval 1"
  "10|tauA3|平均审批 3 年|--budget 900 --carry 3 --horizon-eval auto --tau-approval 3"
  "11|tauA5|平均审批 5 年（最慢）|--budget 900 --carry 3 --horizon-eval auto --tau-approval 5"
  "12|quota2|年度配额 2（更紧）|--budget 900 --carry 3 --horizon-eval auto --quota 2"
  "13|quota6|年度配额 6（更松）|--budget 900 --carry 3 --horizon-eval auto --quota 6"
  "14|build3|建设年限 3 年（全通道）|--budget 900 --carry 3 --horizon-eval auto --build-years 3"
  "15|buildmix|建设年限逐通道分档（情景参数，无实证依据）|--budget 900 --carry 3 --horizon-eval auto --build-years-by-channel 1:5,2:3,3:5,5:3"
  "16|statutory|法定窗口 tau_valid=2 tau_ext=1|--budget 900 --carry 3 --horizon-eval auto --tau-valid 2 --tau-ext 1"
)
NGROUP=${#SCENARIOS[@]}
...
write_status () {
  {
    ...
    if [ -f "$RES/.alldone" ]; then
      echo "## 全部实验已完成（${NGROUP}/${NGROUP}），已打标签 \`batch-v15-complete\`"
    else
...
for spec in "${SCENARIOS[@]}"; do
  ...
  OUT="$BASE/$DIR"; LOG="$RES/g${NUM}_${DIR}.log"; TAG="v15-g${NUM}-${DIR}"
  ...
  write_status
  push_now "批次 v15 第 $NUM/$NGROUP 组：${NAME}（${ST}，耗时 ${hm}）"
  [ "$ST" = "完成" ] && tag_now "$TAG" "v15 第 $NUM 组 ${NAME}：${ITERS} iters × ${EPS} eps × ${RUNS_PER_GROUP} 次，耗时 ${hm}"
done
...
PY
  ...
  push_now "批次 v15 全部 $NGROUP 组完成，含实施类指标与主判据"
  tag_now "batch-v15-complete" "v15 全部 $NGROUP 组完成"
else
